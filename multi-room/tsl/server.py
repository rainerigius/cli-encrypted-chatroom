...
# --------------------- Socket Setup with TLS ---------------------
# Create an SSL context for the server.
# You need to provide 'server.crt' and 'server.key'.
context = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
context.load_cert_chain(certfile="server.crt", keyfile="server.key")

# Create a normal socket and wrap it with SSL
bind_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
bind_socket.bind((HOST, PORT))
bind_socket.listen()
ssl_server_socket = context.wrap_socket(bind_socket, server_side=True)

print(f"Server is running on {HOST}:{PORT} (TLS enabled)")
...
def accept_connections():
    while True:
        client, address = ssl_server_socket.accept()
